YUI.add('moodle-local_lessonexport-sortpages', function (Y, NAME) {

/*global M*/
M.local_lessonexport = M.local_lessonexport || {};
M.local_lessonexport.sortpages = {
    cmid: null,
    groupid: null,
    userid: null,
    sesskey: null,
    ...
    init: function(opts) {
        ...
        sortpages = Y.one('#lesson-sortpages');
    ...
    get_pageid: function(el) {
        ...
        elid = el.get('id');
        pageid = elid.match(/lessonpageid-(\d+)/);
        if (pageid) {
            return pageid[1];
        }
        return null;
    ...
    get_by_pageid: function(pageid) {
        return Y.one('#lessonpageid-'+pageid);
    },
    ...
    show_spinner: function() {
        Y.one('#lesson-sortpages-spinner').addClass('show');
    },

    hide_spinner: function() {
        Y.one('#lesson-sortpages-spinner').removeClass('show');
    },
    ...
    send_reorder: function(drag, destsort) {
        ...
        url = M.cfg.wwwroot + '/local/lessonexport/sortpages_ajax.php';
        params = {
            data: {
                id: this.cmid,
                userid: this.userid,
